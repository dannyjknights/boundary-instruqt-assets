...
locals {
  ...
  cloudinit_config_boundary_instance_worker = {
    write_files = [
      {
        ...
        //content     = file("../files/gpg_pubkeys/hashicorp-archive-keyring.gpg")
        owner       = "root:root"
    ...
    runcmd = [
      ...
      ["sh", "-c", "wget -O- https://apt.releases.hashicorp.com/gpg | sudo gpg --dearmor -o /usr/share/keyrings/hashicorp-archive-keyring.gpg"],
      ["sh", "-c", "echo \"deb [signed-by=/usr/share/keyrings/hashicorp-archive-keyring.gpg] https://apt.releases.hashicorp.com $(lsb_release -cs) main\" | sudo tee /etc/apt/sources.list.d/hashicorp.list"],
